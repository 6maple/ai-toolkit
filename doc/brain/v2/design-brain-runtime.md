# brain v2 Design Verification Index

> **Layer:** non-canonical Phase 5 verification/support document。
> **Status:** non-canonical verification index；**PASS / Design Frozen (2026-08-24, evidence-corrected re-freeze)**。
...
|---|---|---|
| AC2-CONTRACT-001 | Integration + B1/B2/B3 | one public Tool definition source；primitive semantics归 application owner |
| AC2-ANCHOR-001 | B4 + Integration | explicit think返回 shared B4 actual context；generic host用 contract guidance |
| AC2-ANCHOR-002 | Integration | hook直接触发 same B4；真实替代 think 时隐藏 model-visible think |
| AC2-ANCHOR-003 | B4 | restored context明确 prior/latest-evidence authority relation |
| AC2-ANCHOR-004 | B4 + E2/E3 | required core failure fatal；single bad archival local skip；cycle/exposure/Git failure local degradation |
| AC2-SCOPE-001 | B1 + B4 | scope只表达 applicability；不推导 priority/authority |
| AC2-CORE-001 | C1 + B4 | 每 valid scope一个 bounded real core；applicable core resident |
| AC2-CORE-002 | B1/E1 + B4 | session 同时有 core + archival；fresh current session由 anchor materialize |
| AC2-CORE-003 | C1 + B3 | core mutation先完整 validation/capacity check；超限 zero mutation + curation guidance |
| AC2-DISCLOSURE-001 | B4 + B2 | L0 / active discovery / exact cat可自由跳转，无额外读取状态机 |
| AC2-DISCLOSURE-002 | B2 | ls/glob gist；grep content evidence；cat exact document |
| AC2-CAT-001 | B2 | frontmatter/body共用 logical document 1-based line coordinate |
| AC2-CAT-002 | B2 | stable line paging + executable continuation |
| AC2-CAT-003 | B2 | oversized logical line never clipped-as-consumed；lossless host-file escape hatch |
| AC2-MEMORY-001 | C1 + B2/B4 | summary是 document current gist single truth；presentation bounds属于 caller |
| AC2-MEMORY-002 | B1 | role只由 `memories/<role>/` public path表达 |
| AC2-MEMORY-003 | C1 + D2 | importance是 omission-cost enum；只保护 scarce discoverability，不造 match |
| AC2-MEMORY-004 | C1/B3 + B4 | cognition保存必要 epistemic meaning；rendering不洗白 authority |
| AC2-WRITE-001 | B3 + C1/C2/D1 | create=fresh cognition；overwrite=fresh replacement state |
| AC2-EDIT-001 | B3 + C1/D1 | same cognition evolves；whole-content edit仍保持 continuity |
| AC2-MV-001 | B3 + D1 | same archival cognition relocates；cross-scope rebase；destination replacement |
| AC2-EDIT-002 | B3 + E2 | edit mode/object-kind/exact-text validation failure zero mutation |
| AC2-RM-001 | B3 | archival Markdown exit makes cognition inactive；companion cleanup/history are not success prerequisites |
| AC2-FEEDBACK-001 | B3 + D1 | read/discovery ≠ adopt；explicit adopt才形成 validated-use learning |
| AC2-FEEDBACK-002 | B3 + C2 | edit修 cognition；question/resolve只维护 current epistemic challenge |
| AC2-FEEDBACK-003 | B3 | causal attribution/model judgment；无 automatic negative feedback |
| AC2-FEEDBACK-004 | B3 + C2 | question requires challenge；resolve requires current challenge |
| AC2-QUESTIONED-001 | C2 + D2 + B2/B4 | challenge current；passive trust降低但 true discovery/exact read不隐藏 |
| AC2-LEARNING-001 | D1 + D2/B2 | forgetting改变 scarce discoverability，不改变 membership/match truth |
| AC2-TIME-001 | D1 | event-time per-scope cognition cycle；无 wall-clock decay |
| AC2-RETRIEVAL-001 | D1 + B2 | archival substantive cat触发 exact retrieval；list/search/passive display不 refresh |
| AC2-REINFORCEMENT-001 | D1 + B3 | adopt增强 durability；普通 read不增强 |
| AC2-EXPOSURE-001 | D1 + B4 | only actually rendered L0 cues gain passive exposure debt |
| AC2-DISCOVERABILITY-001 | D2 | R + importance floor独立保护，stable deterministic tiebreak |
| AC2-PATH-001 | B1/E1 | one logical public namespace projected to one canonical physical tree |
| AC2-PATH-002 | B1 | only cognition object kinds/directories public；hidden mechanism state不可寻址 |
| AC2-PATH-003 | B1/E1 | cognition tools share one logical grammar + physical containment boundary |
| AC2-PATH-004 | B1/E1 + B2/B3/B4 | exact operations canonicalize before kind/identity decisions；broad aliases dedupe canonical targets；broken/outside failure localized |
| AC2-ABSOLUTE-001 | B1/E1 + Integration | separate permissive workspace-location grammar maps cognition/assets to contained absolute filesystem paths without widening cognition tools |
| AC2-LS-001 | B2 + D2 | direct-child truth first；strength只在 scarce archival presentation排序 |
| AC2-LS-002 | B2 | fixed bounded whole records + refine；no page-2 protocol |
| AC2-GLOB-001 | B2 + D2 | public path-pattern truth first；accessibility不能抹掉 true match |
| AC2-GLOB-002 | B2 | fixed bounded result + refine；no continuation |
| AC2-GREP-001 | B2 | literal/regex truth只来自 applicable memories trees 的 active archival content；core resident、不进入 discovery corpus；summary不伪造 evidence |
| AC2-GREP-002 | B2 | Pi-style bounded grep + explicit refine affordance；无 public pagination state；display不产生 learning transition |
| AC2-GREP-003 | B2 | invalid regex is input error；literal mode remains explicit alternative |
| AC2-HISTORY-001 | E3 + current workspace | cognition success independent of Git；existing commits optional recovery；no parallel history |
| AC2-HISTORY-002 | B4 + E3 | each anchor gives at most one best-effort current-workspace checkpoint opportunity |
| AC2-CONSISTENCY-000 | C1/C2/D1 + E1/B2/B3/B4 | Markdown/core truth；D1/scope-cycle fresh fallback；C2 challenge remains semantic when promised |
| AC2-CONSISTENCY-001 | E2 + B2/B3/B4 | required public state has strong success boundary；incidental learning/history may degrade |
| AC2-CONCURRENCY-001 | E2 | process serial + global semantic coordination give sequentially explainable successes |
| AC2-CONCURRENCY-002 | E2 | only actually shared global current state pays cross-process semantic coordination |
| AC2-FAILURE-001 | E2 | prevalidation zero side effect；catchable failure restore before-state or fail loud |
| AC2-RESTART-001 | E1/E2 | successful current working state survives restart even if never checkpointed |
| AC2-RESTART-002 | E1/E2 + E3 optional | hard kill before success non-goal；working files truth；no WAL/Git-cleanliness gate |
| AC2-LIFECYCLE-001 | D1 + B3 | learning不自动 promotion/demotion/reclassification/status change |
| AC2-LIFECYCLE-002 | B4 | no generic lifecycle candidate subsystem；core/L0只提供语义 affordance |
| AC2-PRESENTATION-001 | B4 | stable owner guidance + dynamic path/summary/status；no internal scores/IDs |
| AC2-PERSISTENCE-001 | B4 + Integration | persistence judgment stays model-owned；no mandatory write hook |
...
```text
prior cognition may leave transcript/context
...
→ reason with current user/evidence + prior cognition
...
|---|---|---|
| prior cognition missing before a new turn | restore before substantive reasoning | B4 + Integration |
| restored prior gains false authority | preserve original meaning/certainty/commitment；latest user/evidence can update it | B4 + C1/B3 |
...
### R1 — Current instruction updates conflicting prior cognition
Prior memory contains an older decision; current user explicitly changes it. Model must treat memory as prior context, follow the current instruction, and update only affected cognition.
...
### R3 — L0 cue leads to appropriate deeper retrieval
Given path + gist but insufficient detail, model can directly cat or choose ls/glob/grep as useful. L0 is not mistaken for complete evidence and no private read-level protocol is required.
...
### R8 — Importance and scope are not relevance/authority
Critical/global cognition is not automatically more relevant or correct. Importance protects scarce discoverability；scope only states future applicability.
...
Hook-injected `<brain_think_context>` is interpreted as prior cognition workspace, not as a new user message or newly observed fact.
...
When a turn updates an existing cognition plus introduces session/project/resident facts, the model maintains the canonical owner, selects future applicability semantically, and does not follow a fixed promotion ladder.
...
- contradictions or avoidable mistakes caused by missing/stale prior cognition；
...
10. **Verification surface** — deterministic behavior有 unit/invariant/integration seam；model semantic behavior有 R1–R15 Replay/Eval，且初始产品取舍有 G1 goal-level effectiveness/context-cost 对照 Eval；
...
Freeze status：**PASS / FROZEN (2026-08-24, evidence-corrected re-freeze)** — current canonical Design closes all 59 Acceptance cases without an identified unresolved Design gap。

### Freeze baseline

Design Freeze applies to the following current v2 truth set:
...
`R1–R15` Representative Replay/Eval and `G1` goal-level effectiveness/context-cost comparison are **verification support**, not additional product requirements. They may produce evidence for later calibration or a deliberate Design reopen, but do not silently mutate the frozen baseline.
...
### 5.1 2026-08-24 Evidence-correction Design Re-freeze
