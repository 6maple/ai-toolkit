# brain 行为需求 v2（BDD Requirements Baseline）

> **状态：BDD Requirements Baseline / Frozen for v2 Acceptance（2026-08-24，evidence-based correction 后重新冻结）。**
...
> v1 [`../v1/bdd-brain-behavior-requirements.md`](../v1/bdd-brain-behavior-requirements.md) 仍是 Frozen Baseline；v2 只继承经过本轮重新裁决的行为。  
> 2026-08-22 曾发生真实磁盘内容损坏：中文被替换为字面 `?`。本版本依据保真的 ChatGPT Share transcript、continuation ledger、checkpoint 与仍完整的 ASCII REQ/Scenario 骨架重建；不声称逐字恢复损坏前的中文，但保持已确认语义与稳定 ID。
...
→ model reasons from prior cognition + latest input + current evidence
...
### BR2-P4：怎样区分 discovery、retrieval 与 validated adoption？
看到 gist、真正读取全文、实际使用并被结果支持是不同认知事件。

### BR2-P5：怎样让新 cognition 在遗忘前有机会持久化，而不把 write 变成机械流程？
...
## REQ2-ANCHOR-001 每个新 turn 的实质处理前，适用的 persistent cognition 应已恢复到当前 context

系统应使适用的 global/project/session core 与 bounded L0 archival cues 在模型进行实质推理前可用。integration mechanism 可随 host 不同，但行为目标相同。

### Scenario ANCHOR2-001-A：没有可靠 hook 的 host 通过 tool description 引导 read-before-think
...
### Scenario ANCHOR2-001-B：支持 hook 的 host 自动恢复 cognition
...
## REQ2-ANCHOR-003 anchor 不需要预先知道未来的 discovery query

L0 是 bounded passive cue，不要求系统预测后续 grep 什么；模型根据当前 task/context 自己形成 retrieval intent。

### Scenario ANCHOR2-003-A：任务产生新的 retrieval intent

**Given** anchor 只暴露 core 与少量 L0 cues  
**When** 推理中出现知识缺口或具体 path/content cue  
**Then** 模型可主动生成 ls/glob/grep/cat 调用继续检索  

## REQ2-ANCHOR-004 cognition restore 是 anchor 的主要结果；辅助 learning/history 失败只缩小本轮学习效果

当适用的 current cognition 本身可被正确恢复时，cycle、retrievability/exposure 等附加 learning state 或 Git history 暂时无法更新，不应让模型失去本来可用的 prior cognition。系统可以丢失本轮对应的辅助 learning/history event，并提供可诊断信息。相反，模型将直接依赖和维护的 primary cognition workspace 必须真实成立：required core 无法正确读取，或可靠 fresh current session 的真实空 `core.md` 无法建立时，anchor 应失败而不是伪造一个可维护的 core。

### Scenario ANCHOR2-004-A：辅助状态更新失败不吞掉可恢复的 prior cognition
...
### Scenario ANCHOR2-004-B：fresh current session 不返回假的 core affordance
...
### Scenario ANCHOR2-004-C：required core 无法正确解释时不伪装成功
...
### Scenario ANCHOR2-004-D：单条坏 archival cognition 只影响自身 passive recall
...
## REQ2-SCOPE-001 global / project / session 表示未来适用范围
...
### Scenario SCOPE2-001-A：天然跨项目的 cognition 可以直接写 global
...
### Scenario RESIDENCY2-001-A：core 无需 discovery 即进入 context

**Given** project scope 适用于当前 turn  
**When** anchor 建立  
**Then** project core 在 bounded contract 内直接进入 context  

### Scenario RESIDENCY2-001-B：archival 不整体常驻
...
### Scenario CORE2-001-A：多个适用 scope 的 core 可同时 anchor
...
### Scenario CORE2-001-B：core mutation 超容量
...
## REQ2-AUTHORITY-001 持久化与 core residency 不自动提升 epistemic authority

memory 是过去认知上下文；保存、重复恢复、进入 core 或被自动注入都不改变它原来能够证明什么。模型使用 remembered content 时应保留其形成时的 meaning、certainty 与 commitment；proposal/hypothesis/uncertain claim 不因 persistence 变成 confirmed fact，assistant recommendation 不因 persistence 变成 user decision。当前理解由 prior cognition、最新用户输入与当前证据共同更新。

### Scenario AUTH2-001-A：当前用户指令可以覆盖冲突历史 cognition
...
### Scenario AUTH2-001-B：persisted hypothesis 不因重复恢复变 confirmed fact

**Given** memory 语义是 proposal/hypothesis  
**When** 多次 anchor/read  
**Then** 仍按 proposal/hypothesis 解释，直到新确认  

### Scenario AUTH2-001-C：assistant recommendation 不因被记住变成用户决定

**Given** earlier assistant 只提出了一个 recommendation / next-step proposal  
**And** 该内容后来被持久化并再次恢复  
**When** 当前 turn 使用这段 prior cognition  
...
## REQ2-DISCLOSURE-001 passive cue、active discovery、exact read 是不同访问事件，不是强制 pipeline
...
### Scenario DISCLOSURE2-001-A：L0 candidate 可直接 exact read
...
### Scenario DISCLOSURE2-001-B：已知 exact path 时直接 read

**Given** context 已有准确 archival public path
**When** 模型需要该 archival cognition
**Then** 可以直接 exact read  

## REQ2-DISCLOSURE-002 L0 只暴露少量 gist，不展开 archival 正文
...
### Scenario DISCLOSURE2-002-A：L0 gist 足够判断是否深读
...
## REQ2-DISCLOSURE-003 active discovery 应提供 memory gist；grep 还必须保留真实 match evidence
...
### Scenario DISCLOSURE2-003-A：ls/glob 返回 path + summary
...
### Scenario DISCLOSURE2-003-B：grep 同时返回 gist 与 evidence
...
## REQ2-READ-002 能在单次 read budget 内容纳的 memory 可以一次完整返回

bounded context 保护不能制造读取仪式。`brain_cat` 按普通 read 语义工作；小 memory 可单次完整返回。

### Scenario READ2-002-A：小 memory 单次完整读取

**Given** 完整 logical document 可放入单次 budget  
**When** `brain_cat` 读取  
**Then** 可以一次返回完整文档  

## REQ2-READ-003 长 memory 可以正常分页，且 continuation 稳定
...
### Scenario READ2-003-A：长 memory 明确存在后续

**Given** memory 超过单次 read budget  
**When** 第一页返回  
**Then** 明确还有后续并提供 continuation  

### Scenario READ2-003-B：未修改 document 时不跳失不重复

**Given** 多页读取期间 document 未变  
**When** 依次读取后续页  
**Then** stable content 不无故跳过或重复  

### Scenario READ2-003-C：超长单行不能被截断后伪装成已完整读取
...
## REQ2-MEMORY-001 每个 active archival memory 都有非空、当前一致的 summary gist
...
### Scenario MEMORY2-001-A：passive cue / active discovery 使用 current summary

**Given** archival memory active  
**When** 经 L0 passive cue 或 active discovery 呈现  
**Then** 使用与 current cognition 一致的非空 summary  

### Scenario MEMORY2-001-B：正文变化后旧 gist 不继续成为真相

**Given** memory content 被合法 edit  
**When** 新 cognition 已生效  
**Then** 后续 discovery 不长期返回冲突旧 summary  

## REQ2-MEMORY-002 cognitive role 由 public memory namespace / path 表达，并描述 recalled cognition 应怎样参与 reasoning/action
...
### Scenario MEMORY2-002-A：path 编码 cognitive role，但不制造该 role 的 epistemic authority
...
### Scenario MEMORY2-002-B：重新分类通过 public destination 表达
...
## REQ2-MV-001 `brain_mv` 只移动明确 archival item，不把 core↔archival 伪装成 mv
...
### Scenario MV2-001-A：archival item 可改变 scope/role
...
### Scenario MV2-001-B：destination 已有 cognition 时由显式 mv 替换
...
### Scenario MV2-001-C：core 不是 `brain_mv` source/destination
...
## REQ2-RM-001 `brain_rm` 使 archival cognition 退出 active memory
...
### Scenario RM2-001-A：rm 后退出 active discovery

**Given** archival memory active  
**When** `brain_rm` 成功  
**Then** 不再出现在 think/ls/glob/grep/normal cat active flow  

### Scenario RM2-001-B：core 不接受 rm
...
## REQ2-FEEDBACK-001 memory access、successful use 与 learning evidence 是不同事件
...
### Scenario FEEDBACK2-001-A：只看到 memory 不等于 successful use
...
### Scenario FEEDBACK2-002-B：correction 不自动降低 importance
...
## REQ2-CORRECTION-001 correction 纠正 current cognition，不永久把 identity 标成 questioned
...
### Scenario CORRECTION2-001-A：明确纠正后得到正常 current cognition
...
### Scenario QUESTIONED2-001-A：L0 降低 questioned proactive priority
...
### Scenario QUESTIONED2-001-B：active discovery 不因 questioned 过滤真实 match
...
### Scenario QUESTIONED2-001-C：exact read 正常读取
...
### Scenario QUESTIONED2-001-D：questioned 不能只剩无解释的状态位
...
## REQ2-LEARNING-001 forgetting 表示 discoverability 下降，不改变 memory truth

长期未真正取回的 active memory 可以更难赢得有限 attention，但仍存在、仍参与真实 match、仍可 exact read。forgetting ≠ delete ≠ match false。

### Scenario LEARNING2-001-A：低 retrievability 在 broad discovery 中更深

**Given** A 很久未真正取回，其他条件等价  
**When** broad bounded discovery 有大量候选  
**Then** A 可更难进入前部窗口  

### Scenario LEARNING2-001-B：具体 cue 仍能找回低 retrievability memory

**Given** memory retrievability 很低  
**When** exact path 或足够具体 query 使其成为真实少量结果  
**Then** 正常发现/读取  

## REQ2-TIME-001 memory evolution 随 cognition cycles 演化，墙上时间本身不构成遗忘
...
### Scenario TIME2-001-A：explicit think 与 hook anchor 是同一种 cycle

**Given** host A explicit think，host B hook  
**When** 各处理一个新用户 turn  
**Then** 各形成一次等价 cognition cycle  

### Scenario TIME2-001-B：没有新 event 时 wall-clock 不单独衰减

**Given** 两周无新 anchor/retrieval/use event  
**When** 两周后重新打开  
**Then** 不只因时间经过自动额外 forgetting  

## REQ2-RETRIEVAL-001 exact read 刷新近期 retrievability；passive cue / active discovery 不等价于 retrieval refresh

exact read 表示 cognition 真正回到 document-level context，可提高近期再取回便利性；仅出现在 cue/list 中不代表完整取回。

### Scenario RETRIEVAL2-001-A：exact read 后近期更易取回
...
### Scenario RETRIEVAL2-001-B：broad active discovery 不批量复习结果

**Given** grep 返回大量 results  
**When** 模型只选少数 exact read  
**Then** 其余仅展示结果不自动获得 retrieval refresh  

## REQ2-REINFORCEMENT-001 validated successful use 可以形成比单纯读取更持久的 future accessibility

successful-use evidence 可使 future retrievability 更 durable；BDD 不要求内部一定有 `stability` 字段或固定公式。

### Scenario REINFORCEMENT2-001-A：反复成功使用的 cognition 衰退更慢
...
## REQ2-EXPOSURE-001 重复无效 L0 passive exposure 产生 anti-monopoly pressure，不惩罚主动 discovery
...
### Scenario EXPOSURE2-001-A：反复 L0 展示未使用时逐渐让位
...
### Scenario EXPOSURE2-001-B：旧 exposure 不惩罚明确 grep match

**Given** memory 曾多次 L0 展示未使用  
**When** 后来主动 grep 真实命中  
**Then** 旧 exposure debt 不隐藏/惩罚该 active-search match  

## REQ2-DISCOVERABILITY-003 retrievability 与 importance 对有限 context allocation 保持独立、单调保护

其他条件等价时，更高 R 或更高 importance 都不应反而更难发现；长期未取回不能单独消除 high importance 保护。BDD 不冻结组合公式。

### Scenario DISCOVERABILITY2-003-A：更高 retrievability 不反向降低 discoverability

**Given** A/B importance 等条件相同，A R 更高  
**When** 竞争 bounded window  
**Then** A 不因 R 更高而位置更差  

### Scenario DISCOVERABILITY2-003-B：更高 importance 不反向降低 discoverability

**Given** A/B R 等条件相同，A omission cost 更高  
**When** 竞争 bounded allocation  
**Then** A 不因 importance 更高而更难发现  

### Scenario DISCOVERABILITY2-003-C：长期未使用不能让 importance 完全失效
...
## REQ2-PATH-001 public path 是模型唯一 cognition address

模型不需要理解 internal state id、physical filesystem path 或第二套 locator。discovery path 必须可直接用于 exact read/mutation。

### Scenario PATH2-001-A：discovery path round-trip 到 exact read

**Given** glob/grep 返回 public path X  
**When** 模型 `brain_cat(X)`  
**Then** 读取同一 cognition  

### Scenario PATH2-001-B：internal identity 不成为第二 public address

**Given** 机制内部有 persistent ID/state key  
**When** memory 对模型呈现  
**Then** 模型仍以 public path 寻址  

## REQ2-PATH-002 applicability scope 由对称 public roots 表达
...
### Scenario PATH2-002-A：同 scope 的 core/archival 共享 root
...
### Scenario PATH2-002-B：session root 带 opaque identifier

**Given** session root 为 `@session/<sid>/`  
**When** 解析 `<sid>`  
**Then** 只作为 session identity，不作自由 path fragment  

## REQ2-PATH-003 archival path 由 scope root + cognitive role + 可递归 item path 组成
...
### Scenario PATH2-003-A：role namespace 是 cognitive-role owner

**Given** `@project/memories/decision/database/migration/rollback-policy.md`  
**When** 判断 cognitive role  
**Then** role 是 `decision`  

### Scenario PATH2-003-B：nested directory 不改变 role

**Given** `@global/memories/skill/coding/typescript/refactor.md`  
**When** item 位于多级目录  
**Then** role 仍是 `skill`  

## REQ2-PATH-004 public namespace 只暴露 cognition objects 与 archival public directories
...
### Scenario PATH2-004-A：猜 mechanism filename 不能通过 brain path 访问

**Given** 模型猜到 `state.json`/`index.json` 等 internal filename  
**When** 通过 public namespace 寻址  
**Then** 不作为 cognition object 暴露  

## REQ2-PATH-005 archival directory、archival item 与 core 是不同 public object kinds

工具仅对合法 kind 生效；判断依据是 public ontology，而不是底层碰巧是 file/directory。

### Scenario PATH2-005-A：directory 可 ls 但不是 archival item
...
### Scenario PATH2-005-B：core 与 archival item 行为不同

**Given** target 是 `<scope-root>/core.md`  
**When** mutation 判断 object kind  
**Then** 按 core contract 处理  

## REQ2-PATH-006 所有 brain tools 对同一 public path 保持同一 cognition 语义
...
### Scenario PATH2-006-A：glob/grep/cat/edit path 语义一致
...
## REQ2-PATH-006A `brain_absolute_path` 提供宽松 workspace location 到真实路径的桥接
...
### Scenario PATH2-006A-A：skill 附件可映射给普通文件工具
...
### Scenario PATH2-006A-B：不存在的附件位置也可映射
...
### Scenario PATH2-007-D：broken、不可访问或越界 alias 只影响真实受影响范围
...
## REQ2-LS-001 `brain_ls` 返回指定 archival memories directory 的 direct children
...
### Scenario LS2-001-A：只返回 direct children
...
### Scenario LS2-001-B：namespace structure 不因 strength 改变

**Given** 真实 child memory R 很低  
**When** 结果量在 budget 内  
**Then** 仍作为真实 direct child 返回

## REQ2-LS-002 `brain_ls` 使用 mechanism-owned bounded output；超额时缩小 path，不提供 page 2

模型不设置 page size。超额必须显式说明并建议进入更具体 directory。

### Scenario LS2-002-A：超额时显式 truncated/refine
...
### Scenario LS2-002-B：不通过 ls page 2 穷举
...
## REQ2-GLOB-001 `brain_glob` 按熟悉 path pattern 发现 archival memory
...
### Scenario GLOB2-001-A：recursive pattern 命中 nested archival paths

**Given** role namespace 下有多级 archival items
**When** glob pattern 覆盖这些 paths
**Then** 返回真实 matching archival paths

### Scenario GLOB2-001-B：真实 path match 不因 state 消失
...
## REQ2-GLOB-002 `brain_glob` bounded output；超额 refine pattern/path，不公开分页

超额显式说明更多匹配；不暴露 model-controlled page size，也不要求 glob continuation。

### Scenario GLOB2-002-A：glob 超额时 refine pattern
...
## REQ2-GREP-001 `brain_grep` 支持熟悉的 literal / regex archival content 匹配
...
### Scenario GREP2-001-C：invalid regex 与有效 query 的 no-match 是不同结果
...
## REQ2-GREP-002 `brain_grep` 使用成熟 grep 的 bounded-result 语义，不建立分页状态
...
### Scenario GREP2-002-A：超额时明确提示截断并给可执行收窄动作
...
### Scenario GREP2-002-B：grep 不建立 public pagination state
...
### Scenario GREP2-002-C：仅展示 grep 结果不产生 learning transition
...
## REQ2-APPROVAL-001 合法 memory mutation 直接形成新 state，不因 project/global scope 强制审批重试

用户已明确删除旧 `none/protect`、`confirmed:true`、`pending-approval` 方向。正常 memory maintenance 不应每次打断用户。

### Scenario APPROVAL2-001-A：长期 scope mutation 不进入 pending approval
...
## REQ2-HISTORY-001 Git 可用时提供辅助版本历史；current cognition 与 Tool success 不依赖 Git
...
### Scenario HISTORY2-001-A：current cognition mutation 不等待 Git 才成立
...
### Scenario HISTORY2-001-B：已有 checkpoint 在 Git 可用时提供额外恢复依据
...
### Scenario HISTORY2-001-C：anchor 只提供一次自然 checkpoint opportunity
...
### Scenario HISTORY2-001-D：Git history 不给模型增加 rationale 输入负担
...
### Scenario HISTORY2-001-E：history failure 是局部降级
...
## REQ2-HISTORY-002 不维护与 Git 平行的 custom history / recycle 机制

`history.jsonl`、`change_history.jsonl`、`memories/history/` recycle、专用 tombstone/deletion-history store 不属于 v2 history/recoverability contract。机制内部仍可有满足当前运行正确性所需的 state/lock/temp 等实现状态，但不能把它们重新扩张成第二套版本历史。

### Scenario HISTORY2-002-A：正常 mutation 不追加 custom audit history
...
## REQ2-CONSISTENCY-000 Markdown / core workspace 是 cognition truth；learning/history state 是可重建或可降级的附加状态
...
### Scenario CONSISTENCY2-000-A：Markdown 存在但 companion 缺失仍可读取
...
### Scenario CONSISTENCY2-000-B：旧 companion 不覆盖新 Markdown
...
### Scenario CONSISTENCY2-000-C：scope-cycle 不可用不把 valid core 变成不存在
...
## REQ2-CONSISTENCY-001 success boundary 跟随请求的业务语义，不由 incidental auxiliary side effect 扩张
...
### Scenario CONSISTENCY2-001-A：显式 state mutation success 后立即 read-your-writes
...
### Scenario CONSISTENCY2-001-B：一个 semantic operation 不形成部分成功
...
### Scenario CONSISTENCY2-001-C：read 的 auxiliary learning refresh 失败不吞掉真实内容
...
## REQ2-CONCURRENCY-001 并发成功结果必须等价于某个合法的顺序执行，不产生额外 silent lost update
...
### Scenario CONCURRENCY2-001-A：并发修改不同 item 不丢成功结果
...
### Scenario CONCURRENCY2-001-B：同一 item 的竞争保持 sequential semantics
...
### Scenario CONCURRENCY2-001-C：跨进程协调只覆盖实际共享的 global scope
...
## REQ2-FAILURE-001 可处理失败不形成一个伪装成正常成功的部分 mutation
...
### Scenario FAILURE2-001-A：预条件/校验失败保持原状态
...
### Scenario FAILURE2-001-B：执行中的可捕获失败不静默留下半状态
...
## REQ2-RESTART-001 普通 restart 保留已成功 cognition；强制终止不承诺未完成 mutation
...
### Scenario RESTART2-001-A：success 后普通 restart 保持 continuity
...
### Scenario RESTART2-001-B：强制终止期间的未完成 mutation 可以丢失
...
## REQ2-LIFECYCLE-001 scope、residency 与 epistemic lifecycle 由 cognition meaning / current cognitive need 决定
...
### Scenario LIFECYCLE2-001-A：高频 session cognition 不自动建议扩大 scope
...
### Scenario LIFECYCLE2-001-B：低 retrievability 不推导 semantic demotion
...
### Scenario LIFECYCLE2-001-C：天然广 scope cognition 无需先积累 promotion evidence
...
## REQ2-DISCOVERY-LEARNING-001 retrieval/use learning 直接作用于 future discoverability，不再产生独立 promotion/demotion signal subsystem
...
### Scenario DISCOVERY-LEARNING2-001-A：successful use 的效果留在 accessibility
...
### Scenario DISCOVERY-LEARNING2-001-B：brain_think 不需要 generic lifecycle candidates
...
## 17. Restored Context 的来源、时间与 Presentation Boundary

## REQ2-CONTEXT-BOUNDARY-001 turn-level anchor 必须可被理解为 restored prior context，而不是对当前 user event 的新解释

无论通过 explicit `brain_think` 还是 host hook 注入，anchor 内容在认知时间上属于处理当前 user event 之前已经存在的 prior state。即使 host 在消息排列上把它插入最新 user message 之后，也必须让模型能够区分“刚发生的 user event”与“为处理它而恢复的 prior cognition”。integration mode 不改变这一语义。

### Scenario CONTEXT-BOUNDARY2-001-A：hook 注入位置较新但语义仍是 prior
...
**And** 把它作为 prior starting context 与该 user message / current evidence 一起形成 current understanding  

### Scenario CONTEXT-BOUNDARY2-001-B：explicit brain_think 与 hook 保持相同 context role

**Given** 一个 host 通过 explicit `brain_think` 建立 anchor，另一个 host 通过 hook 建立等价 anchor  
**When** 模型使用返回 cognition  
**Then** 两种 integration 都表达 restored prior context 的同一时间/认识论角色  
**And** 不因 transport 不同改变 memory 的 authority  

### Scenario CONTEXT-BOUNDARY2-001-C：新输入只更新受影响的 prior cognition

**Given** prior cognition 同时包含多个仍有效约束与一个旧 next-step proposal  
**And** latest user message 只明确改变 next-step direction  
**When** 形成 current understanding  
**Then** 更新与新方向冲突的部分  
**And** carry forward 未被新输入/证据改变的有效 cognition  

## REQ2-PRESENTATION-001 anchor presentation 必须让 cognition role、适用边界与可执行 affordance 足够清楚，而不制造新的 semantic owner

presentation 的稳定行为目标是让模型区分机制 wrapper 与 remembered content、core 与 archival cue、current status 与 memory text，并能理解何时需要进一步 read/search/maintenance。presentation 不应把 runtime ranking 包装成“当前 user query relevance”，也不应通过重复 layer/type/authority labels 建立与 canonical path/semantic state 冲突的第二份真相。具体 XML/HTML-like tag 名、attribute 名、嵌套方式、自然语言措辞与 tool identifier 的排版属于 Design / Public Tool Contract。

### Scenario PRESENTATION2-001-A：candidate 被理解为 recall cue，不是假装 task relevance

**Given** L0 candidate 是在没有 current semantic query 的情况下由 bounded recall mechanism surfaced  
**When** candidate 被注入 anchor  
**Then** presentation 让模型把它理解为需要自行判断是否有用的 archival recall cue  
**And** 不声称 runtime 已经证明它与 latest user request 最相关  

### Scenario PRESENTATION2-001-B：scope 信息表达 applicability，而不是 priority/authority

**Given** global/project/session cognition 同时进入 anchor  
**When** 模型读取它们的 public path / scope meaning  
**Then** 能理解各自未来 applicability boundary  
**And** 不因 scope 更宽、更窄或更靠近 current session 就自动推导更高 epistemic authority  

### Scenario PRESENTATION2-001-C：guidance 放在最小且稳定、真正拥有该语义的 owner
...
**And** 每个 concrete core 保留与自身 path/applicability 对应的 `maintain_when` / `maintain_with` / `archive_when` / `archive_with` affordance，因为这些 decision conditions 与 destination scope 属于该固定 cognition owner  
**And** 不把 scope-specific core guidance 上提成一段要求模型重新映射到具体 path 的泛化说明  

### Scenario PRESENTATION2-001-D：presentation 暴露合法 next action，但不替模型做 semantic decision
...
## REQ2-PERSISTENCE-001 每个有效 cognition cycle 都保留语义性的 persistence judgment opportunity，而不是固定 write step
...
### Scenario PERSISTENCE2-001-A：新形成的 durable cognition 有明确保存机会
...
### Scenario PERSISTENCE2-001-B：当前 working cognition 改变时可维护已有 core
...
### Scenario PERSISTENCE2-001-C：raw ToolResult 不自动变成 memory
...
### Scenario PERSISTENCE2-001-D：已有 persistent cognition 被新证据改变时维护 canonical owner
...
### Scenario PERSISTENCE2-001-E：没有 durable cognition change 时正常不 mutation
