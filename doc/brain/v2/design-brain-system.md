# brain v2 System Design

> **Layer:** System Design
> **Owns:** brain v2 的整体 black-box decomposition、跨黑盒 contracts、state ownership、major control/data flow、failure/concurrency boundary 与 cognition-loop composition。
...
本文只回答 **brain v2 整体怎样组成、各 black box 分别负责什么、它们怎样组合才能完成 Frozen cognition behavior**。
...
brain v2 的目标不是构建通用知识库，而是在模型参数不随项目持续演化、原始 transcript 会压缩/腐化/丢失注意力的条件下，用有限 read-before-think 成本恢复当前任务可能需要的持久 cognition，并给主模型保留自然的主动发现、读取和维护能力。

系统必须闭合这条循环：

```text
persistent prior cognition
        ↓
