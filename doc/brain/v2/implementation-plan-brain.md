# brain v2 Implementation Execution Map

> **Layer:** optional Implementation Plan / non-canonical execution aid。
> **Status:** current execution map derived from the 2026-08-24 evidence-corrected Design re-freeze。
...
v2 是当前 v1 semantics 的 replacement，不默认维护 dual compatibility。旧代码和旧测试只作为 failure/evidence reference；只有新的明确 Requirement 才引入 migration / compatibility。
...
Agent verification
→ R1–R15 representative Replay + G1 effectiveness/context-cost comparison
```
...
让后续 application 只消费 v2 typed logical identity、C1/C2 state 和 E1 storage/codec boundary。

## Canonical inputs

- `design-brain-namespace-storage.md` — B1/E1；
- `design-brain-cognition-state.md` — C1/C2/E1 codec；
- `AC2-PATH-001..004`、`AC2-MEMORY-001..004`、`AC2-CORE-003`。
...
- `AC2-LEARNING-001`、`AC2-TIME-001`、`AC2-RETRIEVAL-001`、`AC2-REINFORCEMENT-001`、`AC2-EXPOSURE-001`、`AC2-DISCOVERABILITY-001`、`AC2-QUESTIONED-001`。
...
- `AC2-HISTORY-001..002`、`AC2-CONSISTENCY-000..001`、`AC2-CONCURRENCY-001..002`、`AC2-FAILURE-001`、`AC2-RESTART-001..002`。
...
`src/persistence/{operation-coordination,cognition-state-store}.ts`、`src/git/*` only as required by E3、`src/runtime/bootstrap.ts`。旧 per-operation staging owner 不属于 v2 target architecture。
...
- `AC2-WRITE-001`、`AC2-EDIT-001..002`、`AC2-MV-001`、`AC2-RM-001`、`AC2-FEEDBACK-001..004`、`AC2-QUESTIONED-001`、`AC2-LIFECYCLE-001`。
...
- `AC2-DISCLOSURE-001..002`、`AC2-CAT-001..003`、`AC2-LS-001..002`、`AC2-GLOB-001..002`、`AC2-GREP-001..003`、`AC2-PATH-004`、`AC2-RETRIEVAL-001`。
...
B2 Acceptance 可以直接通过 fake state/E1/search/auxiliary seams 验证；旧 private L1/L2 retrieval semantics 不再参与 v2。
...
- `AC2-ANCHOR-001..004`、`AC2-CORE-001..002`、`AC2-EXPOSURE-001`、`AC2-PRESENTATION-001`、`AC2-PERSISTENCE-001`、`AC2-HISTORY-002`。
...
直接 application call产生 canonical prior-cognition context；B4 不复制 host/MCP semantics，也不把 Git/auxiliary-learning success变成 restore prerequisite。
...
通过 A1 暴露 Frozen 11-tool contract，并让 v2 application成为 `code/brain` 唯一 production semantic path。

## Canonical inputs

- `design-brain-integration.md` — A1；
- `brain-tools-contract.md`；
- `AC2-CONTRACT-001`、`AC2-ANCHOR-001` 与其他 public Tool cases。
...
production build启动 v2 MCP server并只暴露 Frozen contract；不存在 reachable v1 semantics/compatibility shim。
...
让 `code/brain-dsh-plugin` 成为 thin A2 adapter：调用同一个 B4，并把实际 `AnchorResult.context` 注入 host context，不再拥有第二套 brain contract/workflow。

## Canonical inputs

- `design-brain-integration.md` — A2；
- `AC2-ANCHOR-002..003`、`AC2-PRESENTATION-001`。

## Dependency

T6 behavior + T7 stable v2 server contract。

## Implementation area

`code/brain-dsh-plugin/src/*` 中实际需要的 host/MCP/process lifecycle adapters；删除 vendored Tool contract与旧 dual-hook cognition semantics。

## Verification

A2 owner tests：one host trigger → one B4 result、injected bytes/text == `AnchorResult.context`、diagnostics separate、无 second renderer/rerank/default session、hook mode hide think + expose other 10 tools。再做真实 host + MCP 短会话 E2E。

## Done

plugin 仅承担 host invocation / transport / context injection，不拥有 cognition logic或第二份 public contract。
...
证明 implementation 是 Frozen v2 的翻译，而不是产生第三套语义；完成当前承诺的 CI、real-adapter 与 Agent-level evidence path。

## Canonical inputs

- `acceptance-spec-brain.md` — 59 cases / 140 scenarios；
- `test-design-review-brain.md`；
- `design-brain-runtime.md` — 59/59 owner map、R1–R15、G1；
...
5. 以最低充分方式执行或形成 R1–R15 reproducible Replay evidence，并完成 G1 enabled-vs-control comparison。
...
v2 是 replacement，不是 runtime dual-mode compatibility layer。推荐执行关系只是：

```text
new owner tests + implementation
→ v2 application slices converge
