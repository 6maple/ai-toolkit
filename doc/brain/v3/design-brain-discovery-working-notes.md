# brain v3 — Inherited v2 Discovery / Memory Accessibility Premature Design Notes
...
> **v3 继承说明：** 本文按值保留 v2 working notes，因此正文中的 v2 是历史版本指代；v3 current truth 只由本目录的 BDD / Contract / Acceptance / canonical Design 拥有。
...
- presentation 必须让 restored working context、core、archival cognition summary、status、continuity scope 与合法 next action 足够清楚；candidate summary 足够时可按 cognitive role 直接使用，但 surfacing 不能伪装成 runtime 已证明的 current-query relevance 或 claim validity；
