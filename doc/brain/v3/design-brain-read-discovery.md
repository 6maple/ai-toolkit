# brain v3 Detailed Design — Read & Discovery Application
...
`brain_glob.pattern` 匹配 **B1 canonical full public path string**：
...
`brain_grep.glob?` 不复用该 public-path 坐标；它是相对于每个 selected search root 的 familiar Pi/ripgrep file glob，用来收窄实际搜索的 archival Markdown files。两者名称相同但 owner 和匹配 subject 不同，parameter description 必须就地说明。
...
合法目录没有 direct child 时返回 `no archival cognition entries`；这不是 error，也不返回空 ToolResult。
...
合法查询没有 match 时返回 `no matching archival cognition paths`；这不是 error。
...
合法查询没有 content match 时返回 `no matching archival cognition content`；这与 invalid regex 明确区分。
...
8. line 即使在空 page + mandatory envelope 中也无法完整表示 → stop before this line；`blockedLineNumber = 该 line number` 并记录 `blockedLineUtf8Bytes`，不返回 excerpt、不把该行计作 consumed；
...
Model-facing render 在 `nextOffset` 存在时追加：

```text
next_offset: <nextOffset>
continue_with: brain_cat(path=<canonical-path>, offset=<nextOffset>)
```

EOF、empty/out-of-range 与 representation-blocked page 都不输出 continuation。
...
```text
ls: no archival cognition entries

glob: no matching archival cognition paths

grep: no matching archival cognition content
...
Model-correctable error 保留首行 `error: <code>`，并在下一行提供直接修正动作：invalid glob/regex、invalid numeric argument、edit-style query correction 和 object-kind recovery 都不得只返回裸 code。这里不引入第二套 error code。
...
- no public offset/cursor/continuation；truncated result 通过更窄的 pattern/path/glob 重新搜索；
