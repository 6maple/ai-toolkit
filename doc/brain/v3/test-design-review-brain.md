# brain v3 Test Design Review

> **状态：v3 anchor-semantics delta review complete（2026-08-30）；继承的 v2 review evidence 保留用于追溯。**
> **日期：2026-08-22（继承审查）/ 2026-08-30（v3 增量审查）**  
...
> **历史说明：** §§1–7 继承 v2 在 2026-08-22 至 2026-08-24 的审查记录，供追溯；**它们不是 v3 current contract 的替代 truth**。v3 增量审查见 §8。
...
已覆盖删除 promotion/demotion/generic signals；usage/retrievability 只直接影响 accessibility/discoverability。presentation 重点验证 candidate summary 是可直接使用的 bounded working context、surfacing 不是 relevance/validity proof、scope 只编码 continuity、dynamic item 不重复稳定 schema semantics，以及合法 next action affordance。
...
candidate summary 可按 cognitive role 直接使用，但不被当成 current-query relevance / claim-validity proof
...
## 8. 2026-08-30 v3 anchor-semantics delta review

本节只审查 v3 相对 v2 的行为变化。scope、storage、mutation、learning、discovery、coordination 与 Git owner 没有重开；对应 v2 evidence 由 v3 完整文档集继承。

### 8.1 Reopened behavior and ownership

本轮重新打开并同步的 owner 是：

```text
REQ3-AUTHORITY-001 / REQ3-CONTEXT-BOUNDARY-001 / REQ3-MEMORY-002 / REQ3-PRESENTATION-001
→ AC3-ANCHOR-003 / AC3-MEMORY-002 / AC3-PRESENTATION-001
→ Public Tool Contract model-visible anchor
→ B4 Anchor / Restore Detailed Design
→ R16 representative Agent Replay / Eval
```

变化不是单纯润色。它冻结以下可观察语义：

- restored block 是 user-requested working cognition，不是 optional background；
- restored block 是本轮读取的 current persistent cognition snapshot，不是假定已经分析 latest user message，也不因 persistence 被整体标记成较旧；每条 claim 所描述状态的时间分别判断；
- latest user message 定义当前请求，但 current understanding 需要同时考虑 applicable cognition 与相关证据；
- 不同 claim 按各自可被证据建立或更新的范围判断，不能用实现状态无声抹掉 distinct decision/intention；
- archival 只表示 non-resident，不表示 stale 或低 authority；
- candidate summary 足够时可直接参与，summary 不足或需要 exact detail 时才读取正文；
- shared guidance 与 concrete owner guidance 分层清楚，且每个 concrete core 保留自己的完整 update/archive 映射。

### 8.2 Deterministic verification boundary

CI Contract / structure test应验证稳定结构和行为关系，例如：

- wrapper、namespace、core、archival、role 与 candidate collection 的 ownership 结构存在；
- 三个 concrete core 均携带自己的 `update_when` / `update_with` / `archive_when` / `archive_with`；
- candidate collection 区分直接使用 summary、按需 exact read 与主动 search；
- renderer 不重新引入 scope-as-authority、archival-as-stale 或 relevance-proven claims。

测试不应把整段英文文案做 snapshot，也不应断言某一句自然语言必须逐字不变。属性名属于当前 Public Contract 的 model-visible schema，可以进行 contract-level 结构检查；属性值的目标是语义充分与不自相矛盾，由 review 与 Replay/Eval 共同验证。

### 8.3 Representative model evaluation boundary

R16 必须使用真实模型行为检查，而不是只看 XML 是否包含关键词：

```text
restored cognition:
  established workflow decision + active unfinished intention

current workspace evidence:
  implementation/document status

expected answer:
  同时报告当前实现事实、既定工作方式与仍未完成的意图
  不从磁盘单独重建整个任务
  不把 intention 当成 implementation proof
  summary 足够时不机械调用 brain_cat
```

模型若仍把“记忆”自动降为可忽略背景、把最新读取位置误当绝对 authority，或只复述磁盘状态，则属于 v3 目标行为失败，不应通过继续堆叠流程性强制调用来掩盖。

### 8.4 Review conclusion

BDD / Public Contract / Acceptance / B4 / Verification Index 对 v3 delta 的 owner 与验证边界一致。确定性测试负责 schema/structure/invariant；真实 Replay/Eval 负责“是否确实形成联合理解”这一模型行为。两者缺一不可，但后者未完成不阻止规格与设计冻结，只意味着 implementation validation 尚未结束。
