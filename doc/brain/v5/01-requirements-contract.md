...
Allow the active source root to explicitly relate independent Brain projects and use their **project cognition** without merging ProjectId, ownership, sessions, or default search scope.
...
- Adding relations must not silently expand default archival search or automatically load all related cognition.
...
- If there are no relations and no relation errors, no related-project section is needed.
...
- Omitted-path `brain_glob` / `brain_grep` search only built-in applicable memories. Related memories require explicit `#alias/...` addressing.
